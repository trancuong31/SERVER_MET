...
class MainLogic(QObject):
    ...
    def collect_machine_data(self, ipPLC, ipPort, nameMachine, typeMachine):
        ...
        try:
            ...
            self.plc.typeMachine = typeMachine
            ...
            self.insert_time_default(current_time)
            ...
            print(f'Error thu thập dữ liệu: {ex} lúc {current_time}')
            Config.writeLog(f'Error thu thập dữ liệu: {ex}')
            self.plc.clConnect = 'Mất kết nối'
            self.conn.update_status(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,'0')
            self.conn.update_oracle_machine_status(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,'PAUSE')
            print(f'Đang thử kết nối lại máy {nameMachine} ({ipPLC}:{ipPort}) ')
            time.sleep(3)
            try_connect(3, 5)
        return machines
    def insert_time_default(self, current_time):
        hours = current_time.hour
        minutes = current_time.minute
        seconds = current_time.second
        if minutes == 59 and 40<= seconds <= 50 and self.flag == False:
            if self.plc.clStartIDLE is not None:                            
                idle_time = (current_time - self.plc.clStartIDLE).total_seconds()
                if  idle_time > 1:
                    print(f"Thời gian {current_time} máy {self.plc.clNameMachine} IDLE lúc 59:40s : {idle_time:.0f}s")
                    self.conn.insert_standby_time(self.Config['factory'], self.Config['line'], self.plc.clNameMachine, idle_time)
                    self.plc.clStartIDLE = None
                    self.flag = True
            if self.plc.clStartStopTime1 is not None:
                timeerror = (current_time - self.plc.clStartStopTime1).total_seconds() 
                if timeerror > 1:
                    print(f"Thời gian {current_time} máy {self.plc.clNameMachine} ERROR lúc 59:40s : {timeerror:.0f}s")
            ...
            if self.plc.clStartStopTime is not None:
                time_stop = (current_time - self.plc.clStartStopTime).total_seconds() 
                if time_stop:
                    print(f"Thời gian {current_time} máy {self.plc.clNameMachine} STOP lúc 59:40s : {time_stop:.0f}s")
                    self.plc.clStartStopTime = None 
            ...
            if self.plc.clStartRunTime is not None:
                ...
                if timeRun > 0:
                    print(f"Thời gian {current_time} máy {self.plc.clNameMachine} RUN lúc 59:40s : {timeRun:.0f}s")
                    self.plc.clStartRunTime = None  
        ...
        try: 
            ...
            if word_bit_IDLE[0] == 1 and not has_error:
                if self.plc.clStartIDLE is None:
                    ...
                    self.conn.update_status(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,'2')
                else:
                    self.plc.clIDLE = '1'
                    self.plc.clStatus = 'NORMAL'
                ...
                if self.plc.clStartIDLE is not None:
                    idle_time = (current_time - self.plc.clStartIDLE).total_seconds()
                    print(f"Thời gian {current_time} máy {self.plc.clNameMachine} IDLE: {idle_time:.0f}s")
                    self.conn.insert_standby_time(self.Config['factory'], self.Config['line'], self.plc.clNameMachine, idle_time)
        ...
        try:
            if word_bit_light[0] == 1 and word_bit_light[1] == 1 and word_bit_light[2] == 0 :
                if self.plc.clStartStopTime1 is None:
                    ...
                    self.conn.update_status(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,'3')
                    self.conn.update_oracle_machine_status(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,'WARNING')
                else:
                    self.plc.clGreen = '1'
                    self.plc.clYellow = '1'
                    self.plc.clStatus = 'WARNING'
        ...
        try:
            ...
            if (word_bit_light[1] == 1 and word_bit_light[0] == 0 and word_bit_light[2] == 0 and word_bit_IDLE[0] == 0) or word_bit_light[2] == 1 :                  
                if self.plc.clStartStopTime is None and wordunits_errors[187] == 0:
                    ...
                    self.conn.update_status(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,'4')
                    self.conn.update_oracle_machine_status(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,'ERROR')
                else:
                    self.plc.clYellow = '1'
                    self.plc.clStatus = 'NORMAL'
        ...
        try: 
                ...
                for i, error in enumerate(wordunits_errors):
                    if error == 1:
                        ...
                        if (i + 1) not in self.plc.clStartErrorTime:
                            ...
                            self.conn.insert_error_timeon(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,'ERROR', listErrorCode[i+1],listErrors[i + 1],self.plc.clStartErrorTime[i + 1], self.Config['owner'])
                            self.conn.cnt_insert_error_timeon(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,self.Config['projectName'],self.plc.typeMachine, (i+1), listErrorCode[i+1], self.plc.clStartErrorTime[i + 1] )
            ...
            for i in list(self.plc.clStartErrorTime.keys()):  # Duyệt qua các lỗi hiện có
                ...
                if i <= len(wordunits_errors) and wordunits_errors[i - 1] == 0:
                    ...
                    self.conn.update_error_on(self.Config['factory'], self.Config['line'], self.plc.clNameMachine, listErrorCode[i], end_time)
                    self.conn.cnt_update_error_on(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,listErrorCode[i], end_time)
        ...
        try:
            ...
            if (word_bit_light[0] ==1 and word_bit_light[1] ==0 and word_bit_light[2] ==0 and not has_error and word_bit_IDLE[0] == 0):
                if self.plc.clStartRunTime is None:
                    ...
                    self.conn.update_status(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,'1')
                    self.conn.update_oracle_machine_status(self.Config['factory'], self.Config['line'], self.plc.clNameMachine,'OFF')
    ...
    def threadPLC(self):
        while True:
            for plc in self.plcList:
                machines = self.collect_machine_data(plc["ip"], plc["port"], plc["nameMachine"], plc["typeMachine"])
